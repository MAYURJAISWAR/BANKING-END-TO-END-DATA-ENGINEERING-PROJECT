{{ config(
    materialized='table',
    post_hook=[
        "ALTER TABLE {{ this }} ADD CONSTRAINT pk_account_id PRIMARY KEY(account_id)",
        "ALTER TABLE {{ this }} 
        ADD CONSTRAINT pk_account_id_fk_district_code FOREIGN KEY(district_id) REFERENCES {{ ref('district') }}(district_code)"
    ]
) }}
